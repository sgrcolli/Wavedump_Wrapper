...
int main(int argc,char **argv){
	...
	for (int r=1;r<4;r++){
		...
		if (channel[r]<10)		
			sprintf(filename, "SPE/PMT_NB000%d_HV%d_Dark.root",channel[r],  Gain[r]);
		if (channel[r]>=10 && channel[r] <100)
			sprintf(filename, "SPE/PMT_NB00%d_HV%d_Dark.root",channel[r], Gain[r]);
		if (channel[r]>=100)
			sprintf(filename, "SPE/PMT_NB0%d_HV%d_Dark.root",channel[r],  Gain[r]);
		...
		speData->GetXaxis()->SetTitle("Charge (mv*ns)");	
			...
			// Fit the data.
			double fitXMin=min; //put to 40 instead of 50 Ron Collins 19/09/18
			...
			tf11->SetParameter(3,1000);
			tf11->SetParameter(4,peak);
			...
			area[r] =(abs(tf11->GetParameter(3))+1.0*abs(tf11->GetParameter(3)*tf11->GetParameter(6)))/1.67;
			...
			addGraph(photon1Signal,kGreen+2,tf11->GetParameters(),fitXMin,fitXMax); //changed color to green, Ron Collins 19/09/18 
			...
			// Determined from the user if the results of the fit makes sense
			ta->Run("false");
			
			cout << " ***********************************************"<<endl;
			cout <<"Do the results of the fit look reasonable? (y/n)  "<<endl;
			cout << " ***********************************************"<<endl;
			cin>>converge;
			
			if (converge == 'n'|| converge =='N'){
				cout << " ***********************************************"<<endl;
				cout <<"Enter the Location of the Valley  "<<endl;
				cout << " ***********************************************"<<endl;
				cin>>min;
				
				cout << " ***********************************************"<<endl;
				cout <<"Enter the Location of the Peak "<<endl;
				cout << " ***********************************************"<<endl;
				cin>>peak;
